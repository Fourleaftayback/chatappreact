...
router.get("/allusers", passport.authenticate("userPass", {
  session: false
}), (req, res) => {
  User.find({
    _id: {
  ...
  }).select("-resetPasswordExp -resetPasswordToken").sort("user_name").then(users => {
...
// @access  Private
router.get("/search", passport.authenticate("userPass", {
  session: false
}), (req, res) => {
  let username = req.query.username;
  if (username === undefined || username === "") return res.status(422).json({
